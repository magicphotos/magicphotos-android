...
CartoonEditor::CartoonEditor(QDeclarativeItem *parent) : QDeclarativeItem(parent)
{
    IsChanged        = false;
...
void CartoonEditor::openImage(const QString &image_url)
{
...
void CartoonEditor::saveImage(const QString &image_url)
{
...
void CartoonEditor::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget*)
{
    qreal scale = 1.0;
...
void CartoonEditor::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
...
void CartoonEditor::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
    if (CurrentMode == ModeOriginal || CurrentMode == ModeEffected) {
...
void CartoonEditor::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
    if (CurrentMode == ModeOriginal || CurrentMode == ModeEffected) {
...
CartoonPreviewGenerator::CartoonPreviewGenerator(QDeclarativeItem *parent) : QDeclarativeItem(parent)
{
...
void CartoonPreviewGenerator::openImage(const QString &image_url)
{
    QString image_file = QUrl(image_url).toLocalFile();
...
void CartoonPreviewGenerator::paint(QPainter *painter, const QStyleOptionGraphicsItem*, QWidget*)
{
    qreal scale = 1.0;

    if (CartoonImage.width() != 0 && CartoonImage.height() != 0) {
        scale = width() / CartoonImage.width() < height() / CartoonImage.height() ?
                width() / CartoonImage.width() : height() / CartoonImage.height();
    }
    ...
    QRectF src_rect(0, 0,
                    CartoonImage.width(),
                    CartoonImage.height());
    QRectF dst_rect((width()  - CartoonImage.width()  * scale) / 2,
                    (height() - CartoonImage.height() * scale) / 2,
                    CartoonImage.width()  * scale,
                    CartoonImage.height() * scale);

    painter->drawImage(dst_rect, CartoonImage, src_rect);
