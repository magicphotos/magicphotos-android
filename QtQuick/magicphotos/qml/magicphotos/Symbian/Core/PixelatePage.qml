...
Page {
        ...
        Flickable {
            ...
            onContentWidthChanged: {
                if (contentWidth > 0.0 && initialContentWidth > 0.0) {
                    pixelateEditor.width  = contentWidth;
                    pixelateEditor.height = contentHeight;
                }
            }
    ...
    SaveDialog {
        ...
        onDone: {
            pixelatePage.saveFileUrl = file_url_path + "/" + file_name;

            pixelateEditor.saveImage(pixelatePage.saveFileUrl);
        }
    }
}
